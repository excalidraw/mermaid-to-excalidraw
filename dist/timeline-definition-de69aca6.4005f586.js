...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"node_modules/mermaid/dist/timeline-definition-de69aca6.js":[function(require,module,exports) {
"use strict";
...
const tasks = [];
const rawTasks = [];
const getCommonDb = () => _commonDb573409be.c;
const parseDirective = (statement, context, type) => {
  (0, _utilsD622194a.p)(globalThis, statement, context, type);
};
...
const drawFace = function (element, faceData) {
  ...
  function smile(face2) {
    const arc$1 = (0, _d2.arc)().startAngle(Math.PI / 2).endAngle(3 * (Math.PI / 2)).innerRadius(radius / 2).outerRadius(radius / 2.2);
    face2.append("path").attr("class", "mouth").attr("d", arc$1).attr("transform", "translate(" + faceData.cx + "," + (faceData.cy + 2) + ")");
  }
  function sad(face2) {
    const arc$1 = (0, _d2.arc)().startAngle(3 * Math.PI / 2).endAngle(5 * (Math.PI / 2)).innerRadius(radius / 2).outerRadius(radius / 2.2);
...
const draw = function (text, id, version, diagObj) {
  var _a, _b, _c, _d;
  const conf = (0, _commonDb573409be.g)();
  const LEFT_MARGIN = conf.leftMargin ?? 50;
  (_b = (_a = diagObj.db).clear) == null ? void 0 : _b.call(_a);
  diagObj.parser.parse(text + "\n");
  _commonDb573409be.l.debug("timeline", diagObj.db);
  const securityLevel = conf.securityLevel;
  let sandboxElement;
  if (securityLevel === "sandbox") {
    sandboxElement = (0, _d2.select)("#i" + id);
  }
  const root = securityLevel === "sandbox" ?
  // @ts-ignore d3 types are wrong
  (0, _d2.select)(sandboxElement.nodes()[0].contentDocument.body) : (0, _d2.select)("body");
  const svg = root.select("#" + id);
  svg.append("g");
  const tasks2 = diagObj.db.getTasks();
  const title = diagObj.db.getCommonDb().getDiagramTitle();
  _commonDb573409be.l.debug("task", tasks2);
  svgDraw.initGraphics(svg);
  const sections2 = diagObj.db.getSections();
  _commonDb573409be.l.debug("sections", sections2);
  ...
  sections2.forEach(function (section) {
    ...
    _commonDb573409be.l.debug("sectionHeight before draw", sectionHeight);
    maxSectionHeight = Math.max(maxSectionHeight, sectionHeight + 20);
  });
  let maxEventCount = 0;
  let maxEventLineLength = 0;
  _commonDb573409be.l.debug("tasks.length", tasks2.length);
  for (const [i, task] of tasks2.entries()) {
    ...
    _commonDb573409be.l.debug("taskHeight before draw", taskHeight);
  ...
  _commonDb573409be.l.debug("maxSectionHeight before draw", maxSectionHeight);
  _commonDb573409be.l.debug("maxTaskHeight before draw", maxTaskHeight);
  if (sections2 && sections2.length > 0) {
    sections2.forEach(section => {
      ...
      _commonDb573409be.l.debug("sectionNode", sectionNode);
      const sectionNodeWrapper = svg.append("g");
      const node = svgDraw.drawNode(sectionNodeWrapper, sectionNode, sectionNumber, conf);
      _commonDb573409be.l.debug("sectionNode output", node);
  ...
  _commonDb573409be.l.debug("bounds", box);
  if (title) {
  ...
  (0, _utilsD622194a.s)(void 0, svg, ((_c = conf.timeline) == null ? void 0 : _c.padding) ?? 50, ((_d = conf.timeline) == null ? void 0 : _d.useMaxWidth) ?? false);
};
const drawTasks = function (diagram2, tasks2, sectionColor, masterX, masterY, maxTaskHeight, conf, maxEventCount, maxEventLineLength, maxSectionHeight, isWithoutSections) {
  ...
  for (const task of tasks2) {
    ...
    _commonDb573409be.l.debug("taskNode", taskNode);
    const taskWrapper = diagram2.append("g").attr("class", "taskWrapper");
    const node = svgDraw.drawNode(taskWrapper, taskNode, sectionColor, conf);
    const taskHeight = node.height;
    _commonDb573409be.l.debug("taskHeight after draw", taskHeight);
...
const drawEvents = function (diagram2, events, sectionColor, masterX, masterY, conf) {
  ...
  for (const event of events) {
    ...
    _commonDb573409be.l.debug("eventNode", eventNode);
...
const genSections = options => {
  ...
  for (let i = 0; i < options.THEME_COLOR_LIMIT; i++) {
    ...
    if ((0, _khroma.isDark)(options["lineColor" + i])) {
      options["lineColor" + i] = (0, _khroma.lighten)(options["lineColor" + i], 20);
    } else {
      options["lineColor" + i] = (0, _khroma.darken)(options["lineColor" + i], 20);
    }
  }
  for (let i = 0; i < options.THEME_COLOR_LIMIT; i++) {
    const sw = "" + (17 - 3 * i);
...
},{"./utils-d622194a.js":"node_modules/mermaid/dist/utils-d622194a.js","./commonDb-573409be.js":"node_modules/mermaid/dist/commonDb-573409be.js","d3":"node_modules/d3/src/index.js","khroma":"node_modules/khroma/dist/index.js","@braintree/sanitize-url":"node_modules/@braintree/sanitize-url/dist/index.js","lodash-es/memoize.js":"node_modules/lodash-es/memoize.js","dompurify":"node_modules/dompurify/dist/purify.js","dayjs":"node_modules/dayjs/dayjs.min.js"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/timeline-definition-de69aca6.4005f586.js.map