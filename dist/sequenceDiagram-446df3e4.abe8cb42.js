...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"node_modules/mermaid/dist/sequenceDiagram-446df3e4.js":[function(require,module,exports) {
...
const autoWrap = () => {
  ...
  return (0, _commonDb573409be.g)().sequence.wrap;
};
...
const parseMessage = function (str) {
  ...
  _commonDb573409be.l.debug("parseMessage:", message);
  return message;
};
const parseBoxData = function (str) {
  ...
  const boxData = {
    color,
    text: title !== void 0 ? (0, _commonDb573409be.b)(title.replace(/^:?(?:no)?wrap:/, ""), (0, _commonDb573409be.g)()) : void 0,
...
const addLinks = function (actorId, text) {
  ...
  try {
    let sanitizedText = (0, _commonDb573409be.b)(text.text, (0, _commonDb573409be.g)());
    sanitizedText = sanitizedText.replace(/&amp;/g, "&");
    sanitizedText = sanitizedText.replace(/&equals;/g, "=");
    const links = JSON.parse(sanitizedText);
    insertLinks(actor, links);
  } catch (e) {
    _commonDb573409be.l.error("error while parsing actor link text", e);
  }
};
...
const addProperties = function (actorId, text) {
  ...
  try {
    ...
    _commonDb573409be.l.error("error while parsing actor properties text", e);
  }
};
...
const addDetails = function (actorId, text) {
  ...
  try {
    ...
    _commonDb573409be.l.error("error while parsing actor details text", e);
  }
};
  ...
  if (Array.isArray(param)) {
    ...
    switch (param.type) {
      ...
      case "altEnd":
        ...
        break;
      case "setAccTitle":
        (0, _commonDb573409be.o)(param.text);
        break;
      case "parStart":
...
const db = {
  ...
  parseDirective,
  getConfig: () => (0, _commonDb573409be.g)().sequence,
  clear,
  parseMessage,
  parseBoxData,
  LINETYPE,
...
const drawText = function (elem, textData) {
  let prevTextHeight = 0,
    textHeight = 0;
  const lines = textData.text.split(_commonDb573409be.e.lineBreakRegex);
  const [_textFontSize, _textFontSizePx] = (0, _utilsD622194a.x)(textData.fontSize);
  let textElems = [];
  let dy = 0;
...
const _drawTextCandidateFunc = function () {
  ...
  function byTspan(content, g, x, y, width, height, textAttrs, conf2) {
    ...
    const [_actorFontSize, _actorFontSizePx] = (0, _utilsD622194a.x)(actorFontSize);
    const lines = content.split(_commonDb573409be.e.lineBreakRegex);
...
function boundMessage(_diagram, msgModel) {
  ...
  const lines = _commonDb573409be.e.splitBreaks(message).length;
  const textDims = _utilsD622194a.u.calculateTextDimensions(message, messageFont(conf));
...
const setConf = function (cnf) {
  (0, _commonDb573409be.d)(conf, cnf);
  if (cnf.fontFamily) {
...
function adjustLoopHeightForWrap(loopWidths, msg, preMargin, postMargin, addLoopFn) {
  ...
  if (msg.id && msg.message && loopWidths[msg.id]) {
    ...
    msg.message = _utilsD622194a.u.wrapLabel(`[${msg.message}]`, loopWidth - 2 * conf.wrapPadding, textConf);
    msg.width = loopWidth;
    msg.wrap = true;
    const textDims = _utilsD622194a.u.calculateTextDimensions(msg.message, textConf);
    const totalOffset = Math.max(textDims.height, conf.labelBoxHeight);
    heightAdjust = postMargin + totalOffset;
    _commonDb573409be.l.debug(`${totalOffset} - ${msg.message}`);
  }
  addLoopFn(msg);
...
const draw = function (_text, id, _version, diagObj) {
  ...
  bounds.init();
  _commonDb573409be.l.debug(diagObj.db);
  const diagram2 = securityLevel === "sandbox" ? root.select(`[id="${id}"]`) : (0, _d.select)(`[id="${id}"]`);
    ...
    switch (msg.type) {
      ...
      default:
        try {
          ...
          _commonDb573409be.l.error("error while drawing message", e);
        }
    }
  ...
  _commonDb573409be.l.debug("For line height fix Querying: #" + id + " .actor-line");
  const actorLines = (0, _d.selectAll)("#" + id + " .actor-line");
  ...
  _commonDb573409be.l.debug(`models:`, bounds.models);
};
function getMaxMessageWidthPerActor(actors2, messages2, diagObj) {
  ...
  messages2.forEach(function (msg) {
    if (actors2[msg.to] && actors2[msg.from]) {
      ...
      const wrappedMessage = msg.wrap ? _utilsD622194a.u.wrapLabel(msg.message, conf.width - 2 * conf.wrapPadding, textFont) : msg.message;
      const messageDimensions = _utilsD622194a.u.calculateTextDimensions(wrappedMessage, textFont);
  ...
  _commonDb573409be.l.debug("maxMessageWidthPerActor:", maxMessageWidthPerActor);
...
const getRequiredPopupWidth = function (actor) {
  ...
  for (const key in actor.links) {
    const labelDimensions = _utilsD622194a.u.calculateTextDimensions(key, textFont);
...
function calculateActorMargins(actors2, actorToMessageWidth, boxes2) {
  ...
  Object.keys(actors2).forEach(prop => {
    ...
    if (actor.wrap) {
      actor.description = _utilsD622194a.u.wrapLabel(actor.description, conf.width - 2 * conf.wrapPadding, actorFont(conf));
    }
    const actDims = _utilsD622194a.u.calculateTextDimensions(actor.description, actorFont(conf));
  ...
  boxes2.forEach(box => {
    ...
    if (box.wrap) {
      box.name = _utilsD622194a.u.wrapLabel(box.name, totalWidth - 2 * conf.wrapPadding, textFont);
    }
    const boxMsgDimensions = _utilsD622194a.u.calculateTextDimensions(box.name, textFont);
...
const buildNoteModel = function (msg, actors2, diagObj) {
  ...
  let textDimensions = _utilsD622194a.u.calculateTextDimensions(shouldWrap ? _utilsD622194a.u.wrapLabel(msg.message, conf.width, noteFont(conf)) : msg.message, noteFont(conf));
  const noteModel = {
  ...
  if (msg.placement === diagObj.db.PLACEMENT.RIGHTOF) {
    ...
    textDimensions = _utilsD622194a.u.calculateTextDimensions(shouldWrap ? _utilsD622194a.u.wrapLabel(msg.message, Math.max(conf.width, actors2[msg.from].width), noteFont(conf)) : msg.message, noteFont(conf));
  ...
  if (shouldWrap) {
    noteModel.message = _utilsD622194a.u.wrapLabel(msg.message, noteModel.width - 2 * conf.wrapPadding, noteFont(conf));
  }
  _commonDb573409be.l.debug(`NM:[${noteModel.startx},${noteModel.stopx},${noteModel.starty},${noteModel.stopy}:${noteModel.width},${noteModel.height}=${msg.message}]`);
  return noteModel;
};
const buildMessageModel = function (msg, actors2, diagObj) {
  ...
  if (msg.wrap && msg.message) {
    msg.message = _utilsD622194a.u.wrapLabel(msg.message, Math.max(boundedWidth + 2 * conf.wrapPadding, conf.width), messageFont(conf));
  }
  const msgDims = _utilsD622194a.u.calculateTextDimensions(msg.message, messageFont(conf));
  return {
...
const calculateLoopBounds = function (messages2, actors2, _maxWidthPerActor, diagObj) {
  ...
  messages2.forEach(function (msg) {
    msg.id = _utilsD622194a.u.random({
      length: 10
    });
    switch (msg.type) {
      case diagObj.db.LINETYPE.LOOP_START:
  ...
  bounds.activations = [];
  _commonDb573409be.l.debug("Loop type widths:", loops);
  return loops;
};
const renderer = {
  bounds,
...
},{"./mermaidAPI-3ae0f2f0.js":"node_modules/mermaid/dist/mermaidAPI-3ae0f2f0.js","./commonDb-573409be.js":"node_modules/mermaid/dist/commonDb-573409be.js","d3":"node_modules/d3/src/index.js","./utils-d622194a.js":"node_modules/mermaid/dist/utils-d622194a.js","@braintree/sanitize-url":"node_modules/@braintree/sanitize-url/dist/index.js","stylis":"node_modules/stylis/dist/stylis.mjs","dompurify":"node_modules/dompurify/dist/purify.js","lodash-es/isEmpty.js":"node_modules/lodash-es/isEmpty.js","dayjs":"node_modules/dayjs/dayjs.min.js","khroma":"node_modules/khroma/dist/index.js","lodash-es/memoize.js":"node_modules/lodash-es/memoize.js","process":"node_modules/process/browser.js"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/sequenceDiagram-446df3e4.abe8cb42.js.map